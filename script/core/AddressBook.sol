...
abstract contract AddressBook is CommonBase {
  ...
  function _loadDeployment(TNetwork network, string memory deploymentRoot) internal virtual {
    string memory dirPath = string.concat(deploymentRoot, network.dir());

    try vm.readFile(string.concat(dirPath, "exported_address")) returns (string memory data) {
